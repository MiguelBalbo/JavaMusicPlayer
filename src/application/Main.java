...
public class Main extends Application {
	...
	public void start(Stage primaryStage) {
		try {
			...
			ImageView ivFolderIcn = ivFolderIcn();
			...
			GridPane volumePauseGrd = volumePauseGrd();
			...
			GridPane playerCtrlGrd = playerCtrlGrd();
			...
			btnEscolheArquivo.setOnAction(
				e -> {
					try {
						...
						//corrige player
						if (f != null) {
							refreshPlayer(capa, f, capaAlbumGrd, musicaTagLbl, albumTagLbl, artistaTagLbl, tempoFinalLbl, primaryStage, pauseBtn, tempoAtualLbl, pgBar, volumeSldr);
						}
				...
				btnEscolheArquivo.setOnDragDropped(e -> {
		            ...
		            if (db.hasFiles()) {
		                ...
		                try {
							refreshPlayer(capa, file, capaAlbumGrd, musicaTagLbl, albumTagLbl, artistaTagLbl, tempoFinalLbl, primaryStage, pauseBtn, tempoAtualLbl, pgBar, volumeSldr);
						} catch (Exception e1) {
							e1.printStackTrace();
						}
		            }
	...
	private void refreshPlayer(ImageView capa, File f, GridPane capaAlbumGrd, Label musicaTagLbl, Label albumTagLbl, Label artistaTagLbl, Label tempoFinalLbl, Stage primaryStage, Button pauseBtn, Label tempoAtualLbl, ProgressBar pgBar, Slider volumeSldr) throws Exception {
		...
		tempoFinalLbl.setText((String.format("%02d",(int)m1.duracaoSegundos/60)) + ":" + String.format("%02d",((int)m1.duracaoSegundos%60)));
		
		//toca música
		mc.openMusic(primaryStage,f);
		
		//controles do player
		mc.mscPlayerControles(f, pauseBtn, tempoAtualLbl, pgBar, volumeSldr);
	}

	private GridPane playerCtrlGrd() {
	...
	private GridPane volumePauseGrd() {
